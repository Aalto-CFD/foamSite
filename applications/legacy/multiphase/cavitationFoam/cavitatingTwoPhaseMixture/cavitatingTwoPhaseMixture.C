...
License
    ...
    described the according header file.
...
Foam::cavitatingTwoPhaseMixture::cavitatingTwoPhaseMixture(const fvMesh& mesh)
    : twoPhaseMixture(mesh),
      ...
      nu_(IOobject("nu", mesh.time().name(), mesh), mesh,
          dimensionedScalar(dimKinematicViscosity, 0),
          calculatedFvPatchScalarField::typeName),

      thermodynamicProperties_(
          IOobject("thermodynamicProperties", mesh.time().constant(), mesh,
                   IOobject::MUST_READ_IF_MODIFIED, IOobject::NO_WRITE)),

      psil_("psil", dimCompressibility, thermodynamicProperties_),

      rholSat_("rholSat", dimDensity, thermodynamicProperties_),

      psiv_("psiv", dimCompressibility, thermodynamicProperties_),

      pSat_("pSat", dimPressure, thermodynamicProperties_),

      rhovSat_("rhovSat", psiv_ * pSat_),

      rhol0_("rhol0", rholSat_ - pSat_ * psil_),

      rhoMin_("rhoMin", dimDensity, thermodynamicProperties_),

      p_(IOobject("p", mesh.time().name(), mesh, IOobject::MUST_READ,
                  IOobject::AUTO_WRITE),
         mesh),

      psiModel_(barotropicCompressibilityModel::New(thermodynamicProperties_,
                                                    alphav_)),

      psi_(psiModel_->psi()),

      rho_(IOobject("rho", mesh.time().name(), mesh, IOobject::MUST_READ,
                    IOobject::AUTO_WRITE),
           mesh) {
    alphav_.oldTime();

    rho_ = max(psi_ * p_ + alphal_ * rhol0_ +
...
void Foam::cavitatingTwoPhaseMixture::correct() {
    ...
    alphav_ = max(min((rho_ - rholSat_) / (rhovSat_ - rholSat_), scalar(1)),
                  scalar(0));
    alphal_ = 1.0 - alphav_;
